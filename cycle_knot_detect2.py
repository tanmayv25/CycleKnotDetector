...
class P(da.DistProcess):
    ...
    def setup(self, successors, own_id, all_processors, initiator_id, initiator, no_of_edges, node_type, **rest_1390):
        super().setup(successors=successors, own_id=own_id, all_processors=all_processors, initiator_id=initiator_id, initiator=initiator, no_of_edges=no_of_edges, node_type=node_type, **rest_1390)
    ...
    def run(self):
        ...
        def Manual_testing():
            ...
            for (i, p) in enumerate(ps):
                ...
                self._setup({p}, (successors, (i + 1), ps, graph_config.test['initiator'], graph_config.test['no_of_edges'], 'Unknown'))
            self._start(ps)
        Manual_testing()
