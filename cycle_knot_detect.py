# -*- generated by 1.0.12 -*-
import da
PatternExpr_267 = da.pat.TuplePattern([da.pat.ConstantPattern('Request'), da.pat.FreePattern('sender')])
PatternExpr_372 = da.pat.TuplePattern([da.pat.ConstantPattern('Reply'), da.pat.FreePattern('type'), da.pat.FreePattern('sender'), da.pat.FreePattern('S')])
PatternExpr_764 = da.pat.ConstantPattern('Terminate')
PatternExpr_768 = da.pat.TuplePattern([da.pat.FreePattern(None), da.pat.TuplePattern([da.pat.FreePattern(None), da.pat.FreePattern(None), da.pat.FreePattern(None)]), da.pat.ConstantPattern('Terminate')])
_config_object = {}
import sys
import graph_config
...
class P(da.DistProcess):
    ...
    def __init__(self, procimpl, props):
        ...
        self._events.extend([da.pat.EventPattern(da.pat.ReceivedEvent, '_PReceivedEvent_0', PatternExpr_267, sources=None, destinations=None, timestamps=None, record_history=None, handlers=[self._P_handler_266]), da.pat.EventPattern(da.pat.ReceivedEvent, '_PReceivedEvent_1', PatternExpr_372, sources=None, destinations=None, timestamps=None, record_history=None, handlers=[self._P_handler_371]), da.pat.EventPattern(da.pat.ReceivedEvent, '_PReceivedEvent_2', PatternExpr_764, sources=None, destinations=None, timestamps=None, record_history=True, handlers=[])])

    def setup(self, successors, own_id, **rest_843):
        super().setup(successors=successors, own_id=own_id, **rest_843)
    ...
    def run(self):
        self.initiate()
        super()._label('_st_label_761', block=False)
        _st_label_761 = 0
        while (_st_label_761 == 0):
            _st_label_761 += 1
            if PatternExpr_768.match_iter(self._PReceivedEvent_2, SELF_ID=self._id):
                _st_label_761 += 1
            else:
                super()._label('_st_label_761', block=True)
                _st_label_761 -= 1
        self.output('Done!!!')
    ...
    def _P_handler_266(self, sender):

        def bar(i):
            return ((- 1) * i)
        if (self._state.own_id == 1):
            if self._state.has_sent_cycle_message:
                S = [bar(self._state.own_id)]
            else:
                S = []
            ...
            self.send(('Reply', 'incomplete_search', self._id, [self._state.own_id]), to=sender)
        elif (len(self._state.successors) == 0):
            self._state.earlier_reply_sent = 'leaf'
            self.send(('Reply', 'leaf', self._id, []), to=sender)
        else:
            self._state.parent = sender
            self._state.mode = 'awake'
            self._state.num_suc = len(self._state.successors)
            self.send(('Request', self._id), to=self._state.successors)
    _P_handler_266._labels = None
    _P_handler_266._notlabels = None

    def _P_handler_371(self, type, sender, S):
        ...
        if (self._state.own_id == 1):
            ...
            if (self._state.num_suc == 0):
                ...
                self.output('I am in a ', self._state.result)
            ...
            if (self._state.num_suc == 0):
                ...
                if ((self._state.status == 'leaf') or ((self._state.status == 'incomplete_search') and (not unmarked_elements))):
                    self.send(('Reply', 'leaf', self._id, []), to=self._state.parent)
                ...
                if (self._state.status == 'cycle_only'):
                    self.send(('Reply', 'cycle_only', self._id, []), to=self._state.parent)
                if (self._state.status == 'cycle'):
                    if ((self._state.replies_received['cycle'] == 1) and (self._state.replies_received['cycle_only'] == 0) and (self._state.replies_received['incomplete_search'] == 0) and (self._state.replies_received['leaf'] == 0)):
                        ...
                        self.send(('Reply', 'cycle', self._id, new_set), to=self._state.parent)
                self.send('Terminate', to=self._id)
    _P_handler_371._labels = None
    _P_handler_371._notlabels = None
