# -*- generated by 1.0.12 -*-
import da
PatternExpr_310 = da.pat.TuplePattern([da.pat.ConstantPattern('Request'), da.pat.FreePattern('sender')])
PatternExpr_431 = da.pat.TuplePattern([da.pat.ConstantPattern('message_count'), da.pat.FreePattern('message_count')])
PatternExpr_454 = da.pat.TuplePattern([da.pat.ConstantPattern('Reply'), da.pat.FreePattern('type'), da.pat.FreePattern('sender'), da.pat.FreePattern('S')])
PatternExpr_952 = da.pat.ConstantPattern('Terminate')
PatternExpr_976 = da.pat.ConstantPattern('Terminate_initiator')
PatternExpr_956 = da.pat.TuplePattern([da.pat.FreePattern(None), da.pat.TuplePattern([da.pat.FreePattern(None), da.pat.FreePattern(None), da.pat.FreePattern(None)]), da.pat.ConstantPattern('Terminate')])
PatternExpr_980 = da.pat.TuplePattern([da.pat.FreePattern(None), da.pat.TuplePattern([da.pat.FreePattern(None), da.pat.FreePattern(None), da.pat.FreePattern(None)]), da.pat.ConstantPattern('Terminate_initiator')])
_config_object = {}
import sys
import graph_config
import random
...
class P(da.DistProcess):
    ...
    def __init__(self, procimpl, props):
        ...
        self._events.extend([da.pat.EventPattern(da.pat.ReceivedEvent, '_PReceivedEvent_0', PatternExpr_310, sources=None, destinations=None, timestamps=None, record_history=None, handlers=[self._P_handler_309]), da.pat.EventPattern(da.pat.ReceivedEvent, '_PReceivedEvent_1', PatternExpr_431, sources=None, destinations=None, timestamps=None, record_history=None, handlers=[self._P_handler_430]), da.pat.EventPattern(da.pat.ReceivedEvent, '_PReceivedEvent_2', PatternExpr_454, sources=None, destinations=None, timestamps=None, record_history=None, handlers=[self._P_handler_453]), da.pat.EventPattern(da.pat.ReceivedEvent, '_PReceivedEvent_3', PatternExpr_952, sources=None, destinations=None, timestamps=None, record_history=True, handlers=[]), da.pat.EventPattern(da.pat.ReceivedEvent, '_PReceivedEvent_4', PatternExpr_976, sources=None, destinations=None, timestamps=None, record_history=True, handlers=[])])

    def setup(self, successors, own_id, all_processors, initiator_id, initiator, no_of_edges, node_type, **rest_1438):
        super().setup(successors=successors, own_id=own_id, all_processors=all_processors, initiator_id=initiator_id, initiator=initiator, no_of_edges=no_of_edges, node_type=node_type, **rest_1438)
    ...
    def run(self):
        self.initiate()
        super()._label('_st_label_949', block=False)
        _st_label_949 = 0
        while (_st_label_949 == 0):
            _st_label_949 += 1
            if PatternExpr_956.match_iter(self._PReceivedEvent_3, SELF_ID=self._id):
                _st_label_949 += 1
            else:
                super()._label('_st_label_949', block=True)
                _st_label_949 -= 1
        if (self._state.own_id == self._state.initiator_id):
            ...
            while (_st_label_973 == 0):
                _st_label_973 += 1
                if PatternExpr_980.match_iter(self._PReceivedEvent_4, SELF_ID=self._id):
                    _st_label_973 += 1
                else:
                    super()._label('_st_label_973', block=True)
                    _st_label_973 -= 1
            ...
            if (self._state.total_message_count > (2 * self._state.no_of_edges)):
                self.output('Test case failed', self._state.total_message_count)
        else:
    ...
    def _P_handler_309(self, sender):
        ...
        if (self._state.own_id == self._state.initiator_id):
            ...
            self._state.num_suc = len(self._state.successors)
            self.send(('Request', self._id), to=self._state.successors)
    _P_handler_309._labels = None
    _P_handler_309._notlabels = None

    def _P_handler_430(self, message_count):
    ...
    _P_handler_430._labels = None
    _P_handler_430._notlabels = None

    def _P_handler_453(self, type, sender, S):
        self._state.message_count += 1
            ...
            if (self._state.own_id == self._state.initiator_id):
                ...
                if (self._state.status == 'cycle'):
                    if ((self._state.replies_received['cycle'] == 1) and (self._state.replies_received['cycle_only'] == 0) and (self._state.replies_received['incomplete_search'] == 0) and (self._state.replies_received['leaf'] == 0)):
                        ...
                        self._state.type_sent_with_termination = 'cycle'
                        self.send(('Reply', 'cycle', self._id, new_set), to=self._state.parent)
    _P_handler_453._labels = None
    _P_handler_453._notlabels = None
        ...
        def Functional_testing_with_rings_and_knots():
            for n in range(10, 500, 10):
                for node_type in ['cycle', 'knot']:
                    ...
                    self._start(ps)
                    t1 = time.time()
                    self.output('Type: ', node_type)
                    self.output('Number of nodes: ', n)
                    self.output('Total Simulation Time:', (t1 - t0))
